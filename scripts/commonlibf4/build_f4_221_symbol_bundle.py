#!/usr/bin/env python3
"""Build the curated F4 1.11.221 community public-symbol PDB bundle."""
...
from pdb_publics_f4_221 import (  # noqa: E402
    IDENTITY_JSON,
    PUBLICS_TXT,
    load_publics,
)
...
def build(target: Path, output_dir: Path) -> dict:
    ...
    if tuple(manifest.get("file_version") or ()) != (1, 11, 221, 0):
        raise ValueError("target is not Fallout 4 1.11.221.0")
    records = load_publics(str(target), manifest["sha256"])
    functions = [{"rva": int(row["221"]), "name": row["n"],
                  "source": "ANALYSIS"}
                 for row in records if row["t"] == "func"]
    labels = [{"rva": int(row["221"]), "name": row["n"],
