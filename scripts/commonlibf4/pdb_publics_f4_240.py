"""Load demangled publics from the F4 1.11.240 community symbol PDB.

Source: a community-generated, public-symbol-only
``Fallout4_1_11_240_for_debug.pdb``.  It is not an original compiler debug
...
``refs/f4_240_pdb_publics.txt`` with one symbol per line:
...
PUBLICS_TXT = os.path.join(REFS_DIR, 'f4_240_pdb_publics.txt')
...
def _validate_identity(target_pe_path: str, target_sha256: str) -> dict:
    ...
    if (not re.fullmatch(r'[0-9a-f]{64}', source_hash) or
        ...
        raise ValueError('F4 240 PDB source provenance is invalid')
    ...
    if sha not in allowed or _sha256(target_pe_path) != sha:
        raise ValueError('F4 240 PDB-public dump targets another executable')
    ...
    if (int(matched_target.get('image_size', -1)) !=
        ...
        raise ValueError('F4 240 target layout differs from corpus identity')
    codeview = read_pe_codeview(target_pe_path)
    if (codeview.get('guid') != pdb.get('guid') or
            int(codeview.get('age', -1)) != int(pdb.get('age', -2))):
        raise ValueError('F4 240 PE CodeView identity does not match the PDB dump')
    return identity
...
def load_publics(target_pe_path: str, target_sha256: str) -> List[dict]:
    """Return symbols shaped for ``fallback_symbols_json`` (F4 1.11.240).

    Schema mirrors the FNV pdb_naming output: ``{n, t, sig, 221, src}``
    where ``240`` is the per-version RVA key the generated F4_221 import
    ...
    if len(rows) != int(identity['counts'].get('publics', -1)):
        raise ValueError('F4 240 PDB-public count differs from identity')
    selected, quarantine = _unambiguous_rows(rows)
    expected = {
        key: int(identity['counts'].get(key, -1)) for key in quarantine}
    if quarantine != expected:
        raise ValueError('F4 240 PDB ambiguity counts differ from identity')
    manifest = inspect_pe(target_pe_path, include_sha256=False)
    ...
    for rva, name in selected:
        ...
        out.append({
            ...
            'sig': '',
            '240': rva,
            'src': 'f4_240_community_pdb',
            'confidence': 'medium',
            'verified_entry': executable and rva in runtime_starts,
            'sections': {'240': section['name']},
            'target_sha256': {'240': target_sha256.lower()},
        })
    return out
...
def load_executable_name_rvas(target_pe_path: str,
    ...
    return {
        record['n']: int(record['240']) for record in records
        if record['t'] == 'func' and record.get('verified_entry')
    }


if __name__ == '__main__':
    raise SystemExit('Pass an exact F4 1.11.240 PE through the generator; '
